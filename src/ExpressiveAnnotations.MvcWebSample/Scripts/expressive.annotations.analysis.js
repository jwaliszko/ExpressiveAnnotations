/* expressive.annotations.analysis.js
...
Array.sanatize = function(arr, item) {
...
function RegexMatcher(regex) {
    var _regex = new RegExp(String.format('^{0}', regex));
...
function Lexer(tokenDefinitions) {
    ...
    this.analyze = function (expression, removeEmptyTokens) {
        ...
        if (removeEmptyTokens)
            Array.sanatize(tokens, Token.SPACE);            
        return tokens;
    };
    ...
    var next = function() {
        ...
        throw String.format('Lexer error. Unexpected token started at "{0}".', _expression);
    };
}
...
function InfixToPostfixConverter() {
    ...
    var isInfixOperator = function(token) {
        ...
        return Array.contains(op, token);
    };
    ...
    var containsLeftBracket = function(st) {
        return Array.contains(st, Token.LEFT);
    };
    ...
    var evaluate = function(st) {
        ...
        switch (top)
        {
            ...
            case Token.OR:
                ...
                break;
            default:
                throw String.format('RPN expression parsing error. Token "{0}" not expected.', top);
        }
        return x;
    };
}
